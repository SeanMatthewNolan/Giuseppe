...
from ..projection import match_constants_to_bcs, project_dual, match_states_to_bc, generate_separated_bc_funcs
...
class InteractiveGuessGenerator(tk.Tk):
    def __init__(
            ...
            num_solver: Optional[ScipySolveBVP] = None
    ):
        super().__init__()
        ...
        self.project_dual_button = ttk.Button(
                self.control_panel, text='Project Dual', command=self.project_dual)
    ...
    def compute_bc_res(self):
        self.initial_bc_res = self.bc_funcs[0](self._guess)
        self.terminal_bc_res = self.bc_funcs[1](self._guess)
    ...
    def match_states(self):
        ...
        self.initial_state_editor.values, self.initial_costate_editor.values = match_states_to_bc(
                self.prob, self._guess, location=location, project_costates=True)
    ...
    def project_dual(self):
        self.guess = project_dual(self.prob, self._guess)

    def match_constants(self):
        # self.guess = match_constants_to_bcs(self.prob, self._guess)
        self.guess = match_constants_to_bcs(self.prob.comp_ocp, self._guess)
