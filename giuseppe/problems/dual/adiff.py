...
class AdiffDual(Picky):
    ...
    def __init__(self, source_ocp: SUPPORTED_INPUTS):
        ...
        t = self.args['dynamic'][0]
        x = self.args['dynamic'][1]
        p = self.args['dynamic'][4]
        x_and_p = ca.vcat((x, p))
        costates = self.args['dynamic'][2]
        lam = costates[:self.num_states]
        _nu_0 = self.args['initial'][5]
        _nu_f = self.args['terminal'][5]
        ...
        self.ca_hamiltonian = ca.Function('H', self.args['dynamic'],
                                          (self.adiff_ocp.ca_cost.path(*self.adiff_ocp.args)
                                           + ca.dot(lam, self.adiff_ocp.ca_dynamics(*self.adiff_ocp.args)),),
                                          self.arg_names['dynamic'], ('H',))
        self.ca_costate_dynamics = ca.Function('lam_dot', self.args['dynamic'],
                                               (-ca.jacobian(self.ca_hamiltonian(*self.args['dynamic']), x_and_p),),
        ...
        initial_aug_cost = ca.Function('Phi_0adj', self.args['initial'],
                                       (self.adiff_ocp.ca_cost.initial(*self.adiff_ocp.args)
                                        + ca.dot(_nu_0, psi0(*self.adiff_ocp.args)),),
        ...
        terminal_aug_cost = ca.Function('Phi_fadj', self.args['terminal'],
                                        (self.adiff_ocp.ca_cost.terminal(*self.adiff_ocp.args)
                                         + ca.dot(_nu_f, psif(*self.adiff_ocp.args)),),
        ...
        adj1 = ca.jacobian(initial_aug_cost(*self.args['initial']), t) - self.ca_hamiltonian(*self.args['dynamic'])
        adj2 = ca.jacobian(initial_aug_cost(*self.args['initial']), x_and_p).T + costates
        adj3 = ca.jacobian(terminal_aug_cost(*self.args['terminal']), t) + self.ca_hamiltonian(*self.args['dynamic'])
        adj4 = ca.jacobian(terminal_aug_cost(*self.args['terminal']), x_and_p).T - costates
...
# TODO implement AdiffAlgControlHandler
...
class AdiffDiffControlHandler:
    def __init__(self, adiff_dual: AdiffDual):
        self.adiff_dual = adiff_dual
...
class AdiffDualOCP:
    def __init__(self, adiff_ocp: AdiffOCP, adiff_dual: AdiffDual, control_method: str = 'differential'):
        ...
        if control_method.lower() == 'differential':
            self.control_handler = AdiffDiffControlHandler(self.dual)
        else:
